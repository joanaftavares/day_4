...
class GetSeq():
	...
	def __init__(self, pattern1, pattern2):
		self.pattern1 = self.get_expansion_bases(pattern1)
		self.pattern2 = self.get_expansion_bases(pattern2)
		self.re_pattern1 = re.compile(self.pattern1)
		self.re_pattern2 = re.compile(self.pattern2)
# 		self.re_pattern1_rever_comp = re.compile(self.get_expansion_bases(pattern1, True))
# 		self.re_pattern2_rever_comp = re.compile(self.get_expansion_bases(pattern2, True))
	...
	def __get_seq_by_fasta(self, sequence):
		start = -1
		for iter in re.finditer(self.re_pattern1, sequence):
			start = iter.end()
			break
		
		end = -1
		for iter in re.finditer(self.re_pattern2, sequence):
			end = iter.start()
			break
